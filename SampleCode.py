...
"+block.compute_hash()+" "+str(block_hash.startswith('0' * Blockchain.difficulty)))
...
19
SECURING DATA WITH BLOCK CHAIN AND AI
<th>Profile Date</th><th>Access Control</th>
<th>Gender</th>
<th>Contact No</th>
<th>Address</th>
<th>Block Chain Hashcode</th>
<th>Revenue</th>
</th>
</tr><tr>’
 con = pymysql.connect(host='127.0.0.1',port = 3308,user = 'root', password =
'root', database = 'SecuringData',charset='utf8')
 with con:
 cur = con.cursor()
...
'Hospital2' and password == 'Hospital2':
...
password = 'root', database = 'SecuringData',charset='utf8')
21
SECURING DATA WITH BLOCK CHAIN AND AI
...
SECURING DATA WITH BLOCK CHAIN AND AI
 with con:
 cur = con.cursor()
 cur.execute("select * FROM patients where problem_desc like
'%"+search+"%'")
 rows = cur.fetchall()
 for row in rows:
 arr = row[5].split(" ")
...
SECURING DATA WITH BLOCK CHAIN AND AI
 ...
 x = '{ "Patient_id":"'+str(count)+'", "patient_name":"'+name+'", "age":"'+age+'",
"problem_desc":"'+problem+'", "profile_date":"'+str(current_time)+'",
...
patients(Patient_id,patient_name,age,problem_desc,profile_date,access_data,gender,c
ontact_no,address,blockchain_hash,revenue)
VALUES('"+str(count)+"','"+name+"','"+age+"','"+problem+"','"+current_time+"','"+s
tr(access)+"','"+gender+"','"+contact+"','"+address+"','"+hash+"','"+str(revenue)+"')"
